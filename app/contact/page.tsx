      ...
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
        ...
        >
          ...
          <p className="font-body-lg text-on-surface-variant max-w-2xl leading-relaxed">
            Connect with our team for your next architectural project.
          </p>
        </motion.div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
          <motion.div {...fadeUp} className="bg-surface-container-low p-8 md:p-12 rounded-xl hairline-border">
            <form onSubmit={handleWhatsAppSend} className="flex flex-col gap-8">
              <div className="flex flex-col gap-2">
                ...
                <input required name="fullName" value={formData.fullName} onChange={handleChange} type="text" className="bg-transparent border-b border-secondary/30 py-2 focus:border-secondary outline-none text-on-surface transition-colors" placeholder="John Doe" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="font-label-caps text-[10px] text-secondary uppercase tracking-widest">Email Address</label>
                <input required name="email" value={formData.email} onChange={handleChange} type="email" className="bg-transparent border-b border-secondary/30 py-2 focus:border-secondary outline-none text-on-surface transition-colors" placeholder="john@example.com" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="font-label-caps text-[10px] text-secondary uppercase tracking-widest">Message</label>
                <textarea required name="message" value={formData.message} onChange={handleChange} rows={4} className="bg-transparent border-b border-secondary/30 py-2 focus:border-secondary outline-none text-on-surface transition-colors resize-none" placeholder="How can we help?" />
              </div>
              <button type="submit" className="bg-[#25D366] text-white font-label-caps py-4 mt-4 hover:brightness-110 transition-all uppercase tracking-widest flex items-center justify-center gap-3">
          ...
          <motion.div {...fadeUp} className="flex flex-col gap-12 justify-center">
            <div className="flex flex-col gap-4">
              <h3 className="font-headline-md text-secondary">Global Studio</h3>
